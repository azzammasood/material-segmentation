...
def main():
    ...
    # Instantiating an object of class AutoRough, the default values can be replacing by uncommenting.
    ...
    # Single function to save masks in Outputs directory. It is an accumulation of all functions for each stage.
...
class AutoRough:
    # Set default values
    _numberOfColors = 64
    _pixelCountPercentageThreshold = 0.055
    _maximumDelta = 20
    _minimumDistance = 200
    _minimumOccurence = 0.01
    _widenBrightnessSearchRange = True
    _regardSaturation = True
    _medianFilter = True
    ...
    def __init__(self, iterable=(), **kwargs):
        ...
        self._numberOfColors = None
        self._pixelCountPercentageThreshold = None
        self._maximumDelta = None
        self._minimumDistance = None
        self._minimumOccurence = None
        self._widenBrightnessSearchRange = None
        self._regardSaturation = None
        self._medianFilter = None

        self.reset_default_values()

    def reset_default_values(self):
        self._numberOfColors = AutoRough._numberOfColors
        self._pixelCountPercentageThreshold = AutoRough._pixelCountPercentageThreshold
        self._maximumDelta = AutoRough._maximumDelta
        self._minimumDistance = AutoRough._minimumDistance
        self._minimumOccurence = AutoRough._minimumOccurence
        self._widenBrightnessSearchRange = AutoRough._widenBrightnessSearchRange
        self._regardSaturation = AutoRough._regardSaturation
        self._medianFilter = AutoRough._medianFilter
    ...
    def getColorTag(self, color):
        ...
        if (self._widenBrightnessSearchRange):
            if (brightness < 30):
                tag = "Dark-" + tag
            elif (brightness < 40):
                tag = "Medium-Dark-" + tag
    ...
    def average_colors_of_each_tag(self, quantizedColors, taggedColors, tags):
        ...
        for i in range(len(tags)):
            ...
            for j in range(self._numberOfColors):
    ...
    def get_unique_tags_and_pixelcounts(self, palette, pixelcounts):
        ...
        for i in range(self._numberOfColors):
            col = palette[i]
    ...
    def get_color_palette(self, image):
        ...
        restricted_palette = complete_palette[:self._numberOfColors]
    ...
    def separate_colors_with_high_pixelcount(self, uniqueColors, taggedColors, quantizedColors, aw, ah):
        ...
        for i in range(len(sortedUniqueColors)):
            if (sortedUniqueColors[i][2] / (aw * ah)) > self._pixelCountPercentageThreshold:
    ...
    def black_and_white_masks(self, uniqueColors, quantizedColors, gif_image, aw, ah):
        ...
        for i in range(len(uniqueColors)):
            ...
            if occurence > self._minimumOccurence:
