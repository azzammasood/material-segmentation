...
class AutoRough:
    ...
    def __init__(self, image_path, numberOfColors, pixelCountPercentageThreshold, maximumDelta, minimumDistance, minimumOccurence, widenBrightnessSearchRange, regardSaturation, medianFilter):
        self.image_path = image_path
        self.numberOfColors = numberOfColors
        self.pixelCountPercentageThreshold = pixelCountPercentageThreshold
        self.maximumDelta = maximumDelta
        self.minimumDistance = minimumDistance
        self.minimumOccurence = minimumOccurence
        self.widenBrightnessSearchRange = widenBrightnessSearchRange
        self.regardSaturation = regardSaturation
        self.medianFilter = medianFilter

        self.create_Outputs_folder()
...
def main():
    ...
    dithered_image = target_image.dither_image()

    # Get pixel count of unique colors in dithered image
    gif_image, color_counts, aw, ah, rgb_array = target_image.pixels_per_color(dithered_image)
    print("Dimensions: ", aw, "x", ah, "\n")

    # Color Quantization
    # STAGE 1: Get color palette
    quantizedColors, palette = target_image.get_color_palette(gif_image)

    # STAGE 2: Get tag and pixel count of each color
    tags, taggedColors = target_image.get_unique_tags_and_pixelcounts(palette, color_counts)

    # STAGE 3: For each tag, average colors of that tag
    uniqueColors, quantizedColors, taggedColors = target_image.average_colors_of_each_tag(quantizedColors, taggedColors, tags)

    # STAGE 4: Separate colors with high pixel count
    taggedColors, quantizedColors = target_image.separate_colors_with_high_pixelcount(uniqueColors, taggedColors, quantizedColors, aw, ah)

    # Layers Creation
    # STAGE 1: Create black and white masks of colors with high occurence
    uniqueColors, masks = target_image.black_and_white_masks(uniqueColors, quantizedColors, gif_image, aw, ah)

    # Stage 2: Create colorized masks
    masks_pasted, list_of_masks = target_image.colorize_masks_and_merge_masks(uniqueColors, masks)

    # Stage 3: Create Base Color layer
    base_color_layer = target_image.create_base_layer(masks_pasted, rgb_array, aw, ah)

    # Stage 4: Create Detail layer
    target_image.create_detail_layer(list_of_masks, base_color_layer, rgb_array)
