...
def main():
    ...
    target_image.get_masks() # Single function to save masks in Outputs directory. It is an accumulation of all functions for each stage.
...
class AutoRough:
    ...
    def __init__(self, iterable=(), **kwargs):
        # Set default values
        self.numberOfColors = 64  # The amount of colors chosen from the start of the complete color palette of the input image. These colors are then quantized and used for segmentation purposes. Default value is 64. A higher number gives higher segmentation accuracy but takes more time.
        self.pixelCountPercentageThreshold = 0.055  # Threshold amount used for separation of colors with high number of pixels. from those with low number of pixels. Default value is 0.055. A higher threshold yields fewer masks, filtered by median reduction.
        self.maximumDelta = 20  # Threshold amount used for comparison of color distances between colors with low pixel counts and colors of low pixel counts. A higher value deletes more residue (does not add low pixel count colors to high pixel count colors).
        self.minimumDistance = 200  # Default distance to be compared with maximumDelta. A lower value will delete more residue.
        self.minimumOccurence = 0.01  # Each color in the final set of quantizedColors has its pixel count compared with this threshold. A higher number yields fewer masks.
        self.widenBrightnessSearchRange = True  # If true, color variation within the same hue range will be accepted, resulting in more masks but a more accurate albedo representation.
        self.regardSaturation = True  # Increases accuracy in color quantization, resulting in higher number of masks at the cost of more computation time.
        self.medianFilter = True  # If true, yields fewer masks.
